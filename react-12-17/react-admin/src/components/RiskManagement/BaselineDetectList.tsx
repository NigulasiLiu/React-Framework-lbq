...
class BaselineDetectList extends React.Component<BaselineDetectListProps, BaselineDetectListState> {
        ...
        this.state = {
            ...
            baselineDetectColumns: [
                ...
                },
                {
                    ...
                    sorter: (a: any, b: any) =>
                        parseFloat(b.last_checked) - parseFloat(a.last_checked),
                },
                {
                    title: '操作',
                    dataIndex: 'operation',
    ...
    getIgnoredBLItemCount = (ignoredBLCheckItem_array: { [uuid: string]: string[] }) => {
        return Object.values(ignoredBLCheckItem_array).reduce(
            (count, BLCheckItem) => count + BLCheckItem.length,
            0
        );
    };
    ...
    handleSubmit = (selectedUuids: string[], selectedUuidNameStrings: string[]) => {
        ...
        selectedUuids.forEach((uuid, index) => {
            ...
            if (selectedUuidNameStrings[0]) {
                ...
                // 发送单独的 POST 请求
                axios
                    .post(Once_Task_API + `?job_id=${job_id}`, requestBody, {
                        headers: {
    ...
    renderCurrentPanel(linuxBaseLineCheckOriginData: any[], windowsBaseLineCheckOriginData: any[]) {
        if (
            linuxBaseLineCheckOriginData === undefined ||
            windowsBaseLineCheckOriginData === undefined
        ) {
            return (
            ...
            const api =
                currentPanel === 'windows' ? BaseLine_windows_Data_API : BaseLine_linux_Data_API;
            return (
                <div>
                    <DataDisplayTable
        ...
        return (
            <DataContext.Consumer>
                {(context: DataContextType | undefined) => {
                    ...
                    const hostCheckedPassedRate =
                        1 -
                        ((blLinuxNeedAdjustmentItemCount || 0) +
                            (blWindowsNeedAdjustmentItemCount || 0)) /
                            (blWindowsCheckNameCount + blLinuxCheckNameCount);
