  ...
  async fetchTaskDetails() {
    try {
      ...
      if (taskData !== undefined) {
        ...
        jobIds.forEach(item => { message.info("已存在的jobId有:" + item) })
  ...
  handleSubmit = async (values: any) => {
    ...
    try {
      ...
      const responses = await Promise.all(postDataArray.map((postData) => {
        ...
        return fetch('http://127.0.0.1:5000/api/add_task', {
          method: 'POST',
                ...
                >
                  <Row>
                    ...
                    <Row style={{ width: '100%' }}>
                      ...
                      <Col span={6}>
                        <Form.Item label="参数" name="args" rules={[{ required: false, message: '请输入参数' }]}>
                          <Input placeholder="示例:('task1');参数仅支持字符串，整数，浮点数，布尔类型" />
                        </Form.Item>
                      </Col>
                      <Col span={1}>
                      </Col>
                      <Col span={6}>
                        <Form.Item label="关键字参数" name="kwargs" rules={[{ required: false, message: '请输入关键字参数' }]}>
                          <Input placeholder="示例:(isEmpty=True);参数仅支持字符串，整数，浮点数，布尔类型" />
                        </Form.Item>
                      </Col>
                    </Row>
